Carre.GameLogic = {
  ...
  init : function() {
    ...
    this.eachFrame.push(function(elapsedTime) {
      ...
      if ( p.collided ) {
        while ( i-- ) { Util.addParticle( _this, p, p.collisionPoints.head1 ); }

      // create approx 1 particle every 5 frames otherwise
      } else if ( Math.random() * 5 > 4 ) {
        Util.addParticle( _this, p, p.collisionPoints.head1 );
      }
    });
