...
Util.addParticle = function( GameLogic, p, head, collided ) {
  GameLogic.get("particle", {
    x: p.x + head.x + Math.round(Math.random() * 16 * ( Math.round(Math.random()) ? 1 : -1)),
    y: p.y + head.y - 1,
    vy: - (.2 + Math.random() * .6),
    vx: collided === "left" ? 1  + Math.random() * .5 :
      collided === "right" ? -(1  + Math.random() * .5) :
      (Math.round(Math.random()) ? 1 : -1) * ( collided ? 2 : .2 ),
    width: 24,
    height: 24,
    noload: true
  });
