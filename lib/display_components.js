...
Carre.DisplayComponents.Timer = {
  load: function() {
    ...
    $timer.style.left = canvasBCR.left + 700 + "px";
  },
  display: function(gameObject, renderingContext, elapsedTime) {
    var time = ((Date.now() - gameObject.loadLevelTime) / 1000 + "").split(".");
    document.getElementById("timer").innerHTML = "<b id=ts>"+time[0]+"</b>.<b id=tms>"+Math.round(time[1]/10)+"</b>";
  }
}
