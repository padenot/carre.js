...
Carre.DisplayComponents.Particle = {
  resources : "assets/sprites/particle.png",
  sprite : null,
  load: function() {
    var _this = this;
    Util.loadImageAsset(this.resources, function(sprite, cache) {
  ...
  display: function(gameObject, renderingContext, elapsedTime) {
    ...
    renderingContext.drawImage(this.sprite, Math.round(gameObject.x - camera_x - gameObject.width /2), Math.round(gameObject.y - camera_y - gameObject.height /2), Math.round(gameObject.width), Math.round(gameObject.height));
