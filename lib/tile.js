...
Carre.Tile = {
  ...
  generateColorIndexes : function(colorNumber) {
    ...
    this.mapData = [];
    for (var i in this.map.layers[0].data) {
      var tId = this.map.layers[0].data[i];
   ...
   * @return void
   */
  drawTile : function(id, dest_x, dest_y) {
    var tId = this.map.layers[0].data[id];
    ...
    var tw = this.map.tilewidth,
        th = this.map.tileheight,
        tileData = this.mapData[id],
        source_y = tileData.y + tileData.color * this.mapHeight;
        ...
        for (y = tile_offset_y; y < tile_offset_y + vp_t_y; y++) {
          for (x = tile_offset_x; x < tile_offset_x + vp_t_x; x++) {
            ...
            if (data[tile_index] !== 0 && this.isInMap(x, y, mw, mh))  {
              this.drawTile(tile_index, dest_x, dest_y);
            }
