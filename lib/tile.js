...
Carre.Tile = {
  ...
  load : function(map, collision) {
    ...
    Util.xhr(map, "GET", null, function(result) {
      ...
      _this.getBackdrop.bind(_this)(Carre.settings.vignette, Carre.settings.backdrop, Carre.settings.backdrop_logo);
      _this.getLayers.bind(_this)(_this.map.layers);
      _this.generateColorIndexes(_this.map.properties.colorNumber);
    }, function() {
   ...
   * @return void
   */
  getBackdrop : function(vignette, backdrop, cpu) {
    var _this = this;
   ...
   *
   * @param colorNumber Number of colors the tileset has.
   * @return void
   */
  generateColorIndexes : function(colorNumber) {
    ...
    this.mapHeight = this.map.tilesets[0].imageheight / this.colorNumber;
  ...
  drawBackdrop : function(x, y) {
    ...
    for (var i = 0; i < this.backdrop.length; i++) {
      Carre.c.drawImage(this.backdrop[i], x/(i+2), y/(i+2), Carre.settings.width, Carre.settings.height, 0, 0, Carre.settings.width, Carre.settings.height);
    }
    Carre.c.drawImage(this.cpu, x/4, y/4, Carre.settings.width, Carre.settings.height, 0, 0, Carre.settings.width, Carre.settings.height);
    Carre.c.drawImage(this.vignette, 0, 0, Carre.settings.width, Carre.settings.height, 0, 0, Carre.settings.width, Carre.settings.height);
  },
