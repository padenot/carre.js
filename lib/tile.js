...
Carre.Tile = {
  ...
  generateColorIndexes : function(colorNumber) {
    ...
    for (var l in this.map.layers) {
      ...
      for (var i in data) {
        ...
        if (tId === 0 && typeof this.mapData[i] !== "undefined" && this.mapData[i] !== null) {
          continue;
        } else if (tId === 0) {
          var source_x = 0,
              source_y = 0,
   ...
   * @return void
   */
  drawTile : function(id, tId, dest_x, dest_y) {
    if (tId === 0) {
      return;
    }
    var tw = this.map.tilewidth,
      ...
      for (y = tile_offset_y; y < tile_offset_y + vp_t_y; y++) {
        for (x = tile_offset_x; x < tile_offset_x + vp_t_x; x++) {
          ...
          if (data[tile_index] !== 0 && this.isInMap(x, y, mw, mh))  {
            this.drawTile(tile_index, data[tile_index], dest_x, dest_y);
          }
