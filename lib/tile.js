...
Carre.Tile = {
  ...
  drawBackdrop : function(x, y) {
    if (Carre.settings.quality === "HQ") {
      Carre.c.fillStyle = "rgb(50,50,50)";
      Carre.c.fillRect(0,0,800,600);
      for (var i = 0; i < this.backdrop.length; i++) {
        Carre.c.drawImage(this.backdrop[i],  0, 0, this.backdrop[i].width, this.backdrop[i].height, -x/(i+1), -200-y/(i+1), Carre.settings.width*2, Carre.settings.height*2);
      }
      Carre.c.drawImage(this.cpu, 0, 0, this.cpu.width, this.cpu.height, 200-x/2, -y/2, this.cpu.width, this.cpu.height);
      Carre.c.drawImage(this.vignette, 0, 0, this.vignette.width, this.vignette.height, 0, 0, Carre.settings.width, Carre.settings.height);
    } else {
      Carre.c.fillRect(0,0,800,600);
      Carre.c.drawImage(this.backdroplq, 0, 0, this.backdroplq.width, this.backdroplq.height, -150-x/4, -300-y/4, Carre.settings.width*2, Carre.settings.height*2);
    }
  },
